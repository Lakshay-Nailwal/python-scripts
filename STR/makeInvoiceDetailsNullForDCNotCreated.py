...
INPUT_CSV = "/Users/lakshay.nailwal/Desktop/updatedScripts/STR/CSV_FILES/purchase_issues_with_invalid_invoice_v10.csv"
...
def getPurchaseIssueInvoice(db_name, purchaseIssueId):
    connection = create_db_connection(db_name)
    cursor = connection.cursor(pymysql.cursors.DictCursor)
    cursor.execute(
    ...
    purchaseIssueInvoice = cursor.fetchone()
    cursor.close()
    connection.close()
    return purchaseIssueInvoice
...
def makeInvoiceDetailsNullFromPR():
    with open(INPUT_CSV, newline='') as infile:
        ...
        for row in reader:
            ...
            if purchaseIssues:
                for purchaseIssue in purchaseIssues:
                    ...
                    if isValidInvoice == False:
                        ...
                        prInvoiceData = getPurchaseIssueInvoice(db_name, purchaseIssue['id'])
                        if prInvoiceData:
                            prInvoiceDataList.append(prInvoiceData)
                            deletePurchaseIssueInvoiceQuery = (
            ...
            if prInvoiceDataList:
                prInvoiceHeader = prInvoiceDataList[0].keys()
                append_to_csv("prInvoiceDataListbackUP.csv", prInvoiceHeader, prInvoiceDataList, OUTPUT_DIR)

    save_to_csv(
        "makeInvoiceDetailsNullFromPR_Output_v13.csv",
